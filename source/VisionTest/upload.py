...
subparser_dsp = subparsers.add_parser('dsp', help='Upload DSP files')
subparser_dsp = subparsers.add_parser('bridge', help='Upload bridge files')
subparser_dsp = subparsers.add_parser('log', help='pouet')
subparser_dsp = subparsers.add_parser('pictures', help='pouet')
...
elif args.command == 'dsp':
	execute_command("mkdir -p /data/video/dsp")
	ftp.storbinary("STOR dsp/cexec.out", file("target/MPU/cexec.out", "rb"))
	ftp.storbinary("STOR dsp/vision.out", file("target/MPU/vision.out", "rb"))
	ftp.storbinary("STOR dsp/ping.out", file("target/MPU/ping.out", "rb"))
	ftp.storbinary("STOR dsp/photos.out", file("target/MPU/photos.out", "rb"))
	ftp.storbinary("STOR dsp/ddspbase_tiomap3430.dof64P", file("target/DSP/ddspbase_tiomap3430.dof64P", "rb"))
	execute_command("cd /data/video/dsp")
	execute_command("chmod +x cexec.out")
	execute_command("chmod +x ping.out")
	execute_command("chmod +x vision.out")
	execute_command("chmod +x photos.out")
	print("Done")
elif args.command == 'bridge':
	execute_command("mkdir -p /data/video/dsp/bridge")
	ftp.storbinary("STOR dsp/bridge/installBridge", file("bridge_binaries/installBridge", "rb"))
	ftp.storbinary("STOR dsp/bridge/uImage", file("bridge_binaries/uImage", "rb"))
	ftp.storbinary("STOR dsp/bridge/bridgedriver.ko", file("bridge_binaries/dspbridge/bridgedriver.ko", "rb"))
elif args.command == "log":
	execute_command("cd /data/video/dsp")
	file = open("log","wb")
	ftp.retrbinary("RETR dsp/log", file.write)

elif args.command == "pictures":
	execute_command("cd /data/video/dsp")
	i=0
	while i<180:
		print('Get picture '+str(i))
		file = open("pictures/"+str(i),"wb")
		ftp.retrbinary("RETR dsp/"+str(i), file.write)
		i = i+1
