...
static int RunTask(struct DMMCOPY_TASK *copyTask,FILE *inFile, FILE *outFile);
...
static int InitializeProcessor(struct DMMCOPY_TASK *copyTask)
{
	...
	INT procId = 0;
	/* Attach to DSP */
	while (DSP_SUCCEEDED(DSPManager_EnumProcessorInfo(index,&dspInfo, (UINT)sizeof(struct DSP_PROCESSORINFO),&numProcs))) {
...
static int InitializeNode(struct DMMCOPY_TASK *copyTask)
{
	...
	if (DSP_SUCCEEDED(status)) {
		status = DSPNode_Allocate(copyTask->hProcessor, &uuid, pArgs, &nodeAttrIn, &copyTask->hNode);
		if (DSP_SUCCEEDED(status)) {
...
static int RunTask(struct DMMCOPY_TASK *copyTask, FILE *inFile, FILE *outFile)
{
	...
	//Create Send segment
	key_t key = 42;
	int shmid = shmget(key,DEFAULTBUFSIZE,IPC_CREAT|666);
	if(shmid==-1)
	{
		perror("shmget");
		exit(1);
	...
	//Attach segment
	aBufferSend = shmat(shmid,NULL,0);
	if (aBufferSend == (char *)(-1)) 
	{
		perror("shmat");
		exit(1);
	}
	printf("aBufferSend: 0x%x\n",aBufferSend);

	//Create receive segment
	key = 43;
	shmid = shmget(key,DEFAULTBUFSIZE,IPC_CREAT|666);
	if(shmid==-1)
	{
		perror("shmget");
		exit(1);
	...
	//Attach segment
	aBufferRecv = shmat(shmid,NULL,0);
	if (aBufferSend == (char *)(-1)) 
	{
		perror("shmat");
		exit(1);
	...
	if (!aBufferSend || !aBufferRecv) {
		...
		status = DSPProcessor_ReserveMemory(copyTask->hProcessor, ulSendResv, &dspAddrSend);
		if (DSP_SUCCEEDED(status)) {
	...
	if (DSP_SUCCEEDED(status)) {
		...
		status = DSPProcessor_ReserveMemory(copyTask->hProcessor, ulRecvResv, &dspAddrRecv);
		if (DSP_SUCCEEDED(status)) {
			fprintf(stdout,"DSPProcessor_ReserveMemory succeeded. dspAddrdRecv = 0x%x \n",(UINT)dspAddrRecv);
		} else {
			fprintf(stdout, "DSPProcessor_ReserveMemory failed. Status = 0x%x\n", (UINT)status);
		}
	}
	if (DSP_SUCCEEDED(status)) {
		...
		status = DSPProcessor_Map(copyTask->hProcessor, (PVOID)aBufferSend,  ulSendBufferSize, dspAddrSend, &aDspRecvBuffer, 0);
		if (DSP_SUCCEEDED(status)) {
	...
	if (DSP_SUCCEEDED(status)) {
		...
		status = DSPProcessor_Map(copyTask->hProcessor, (PVOID)aBufferRecv,  ulRecvBufferSize, dspAddrRecv, &aDspSendBuffer,0);
		if (DSP_SUCCEEDED(status)) {
			fprintf(stdout, "DSPProcessor_Map succeeded.\n");
		} else {
			fprintf(stdout, "DSPProcessor_Map failed. Status = 0x%x\n",(UINT)status);
	...
	if (DSP_SUCCEEDED(status)) {
		...
		fprintf(stdout, "Sending DMM BUFs to DSP cmd=SETUP, DspRecvBuf=0x%x, DspSendBuf=0x%x \n",(UINT)aDspRecvBuffer, (UINT)aDspSendBuffer);
	...
	if (DSP_SUCCEEDED(status)) {
		...
		DSPProcessor_FlushMemory(copyTask->hProcessor,(PVOID)aBufferSend, DEFAULTBUFSIZE,0);
		DSPProcessor_FlushMemory(copyTask->hProcessor,(PVOID)aBufferRecv, DEFAULTBUFSIZE,0);
		do {
			/* Read a block of data from the input file */
			cBytesRead = fread((PSTR)aBufferSend, sizeof(BYTE),DEFAULTBUFSIZE, inFile);
			if (cBytesRead > 0) {
				...
				if (DSP_FAILED(status)) {
					 ...
					 */
					status = DSPNode_GetMessage(copyTask->hNode, &msgFromDsp, DSP_FOREVER);
					if (DSP_SUCCEEDED(status)) {
						/* Go ahead and flush here */
						...
						for (j = 0;j < cBytesRead;j++) {
							if (((unsigned char *)aBufferSend)[j] !=
								...
								fprintf(stdout,"Sent and received buffers don't match \n");
						 ...
						 * the output file
						 */
						fwrite((PSTR)aBufferRecv, sizeof(BYTE), cBytesRead, outFile);
					} else {
	...
	status = DSPProcessor_UnReserveMemory(copyTask->hProcessor, (PVOID)dspAddrSend);
	if (DSP_SUCCEEDED(status)) {
	...
	/* Unreserve DSP virtual memory */
	status = DSPProcessor_UnReserveMemory(copyTask->hProcessor,  (PVOID)dspAddrRecv);
	if (DSP_SUCCEEDED(status)) {
		fprintf(stdout, "DSPProcessor_UnReserveMemory succeeded.\n");
	} else {
		fprintf(stdout, "DSPProcessor_UnReserveMemory failed. Status = 0x%x\n", (UINT)status);
