...
INT main(INT argc, CHAR * argv[])
{
	...
	if (fError) {
		...
		if (DSP_SUCCEEDED(status)) {
			printf("DSPProcessor_Attach succeeded.\n");
			status = DSPProcessor_Stop(hProc);
			if (DSP_SUCCEEDED(status)) {
				printf("DSPProcessor_Stop succeeded.\n");
				status = DSPProcessor_Load(hProc,argc,(CONST CHAR **)argv,NULL);
				if (DSP_SUCCEEDED(status)) {
					printf("DSPProcessor_Load succeeded.\n");
					status = DSPProcessor_Start(hProc);
					if (DSP_SUCCEEDED(status)) {
						fprintf(stdout,"DSPProcessor_Start succeeded.\n");
					} else {
						printf("DSPProcessor_Start failed: 0x%x.\n", status);
					}
				} else {
					printf("DSPProcessor_Load failed: 0x%x.\n",status);
				}
				DSPProcessor_Detach(hProc);
			}else{
			    printf("DSPProcessor_Stop failed: %d.\n",status);
			}
		} else {
			printf("DSPProcessor_Attach failed: 0x%x.\n",status);
		}
	}
	if (!fScriptable) {
