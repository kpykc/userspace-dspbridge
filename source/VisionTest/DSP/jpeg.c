...
int create_svs_jpeg_header(unsigned char* restrict jpegbuf, int32_t size, int w)
{
...
void (*read_format) (JPEG_ENCODER_STRUCTURE * restrict jpeg_encoder_structure, uint8_t * restrict input_ptr);

void initialization (JPEG_ENCODER_STRUCTURE * restrict jpeg, uint32_t image_format, uint32_t image_width, uint32_t image_height)
{
...
uint8_t* encodeMCU (JPEG_ENCODER_STRUCTURE * restrict jpeg_encoder_structure, uint32_t image_format, uint8_t * restrict output_ptr)
{
    levelshift (Y1);
    DCT (Y1);
...
void levelshift (int16_t* const data)
{
    ...
    int16_t *ptr = data, *end = data+64;
    for(ptr; ptr<end;ptr+=2)
    {
		*((int*)(ptr)) = _sub2(*((int*)(ptr)), 8388736);
    }
}

/* DCT for One block(8x8) */
void DCT (int16_t * restrict data)
{
...
uint8_t* huffman (JPEG_ENCODER_STRUCTURE * restrict jpeg_encoder_structure, uint16_t component, uint8_t * restrict output_ptr)
{
    uint16_t i;
...
uint8_t* close_bitstream (uint8_t * restrict output_ptr)
{
    uint16_t i, count;
    uint8_t *ptr;
...
uint8_t *write_markers (uint8_t * restrict output_ptr, uint32_t image_format, uint32_t image_width, uint32_t image_height)
{
...
void read_422_format (JPEG_ENCODER_STRUCTURE * restrict jpeg_encoder_structure, uint8_t * restrict input_ptr)
{
    int32_t i, j;
