...
void multiplyImages( int* restrict ImA, int* restrict ImB,  int* restrict ImC, int width, int height)
{
  short x,y;
  unsigned int ix;
  ...
  for(x = 0; x < IMG_WIDTH; x++)
  {
   #pragma UNROLL(2)
    for(y = 0; y < IMG_HEIGHT; y++)
    {
      ...
      //DY[ix] = dy;
		ix = int_index(x,y);
		getGradientPixelWH(frame_buf, x, y, &DX[ix], &DY[ix]);
    }
  }
}
...
void smoothGaussian( int* restrict Src,  int* restrict Dst)
{
...
int getSumPatch( int* restrict Patch, int size)
{
...
void MatVVMul(float* MVec, int** Mat,  float* restrict Vec, int MatW, int MatH)
{
  unsigned int i;
  unsigned int j;
...
void ScaleAdd(float* Mat3, float* Mat1, float Scale,  int* restrict Mat2, int MatW, int MatH)
{
  unsigned int i;
  unsigned int j;
  unsigned int ii;
...
void svdSolve( float* restrict x_svd, int **u, int m, int n, int * restrict b)
{
	// SVD
	int i, j;
	int ii;
...
void quick_sort (float * restrict a, int n) {
	float t,p,*l,*r;
    if (n < 2)
        return;
    p = a[n / 2];
...
//	}
	...
	max_new_points = (*count < max_count - *flow_point_size) ? *count : max_count - *flow_point_size; //flow_point_size = [0,25]
	...
	for(i = 0; i < max_new_points; i++)
	{
		...
		{
			// distance squared:
			distance2 = (detected_points[i].x - flow_points[j].x)*(detected_points[i].x - flow_points[j].x) + (detected_points[i].y - flow_points[j].y)*(detected_points[i].y - flow_points[j].y);
