 ...
 */
/*
 *  ======== dma4/acpy2_omap_instrumentation.c ========
...
#define _DMA4_MAX55CHANNELS 32

struct _ChanStat  _ChanStats[_DMA4_MAX55CHANNELS];
 ...
 *  Get channel stats regarding DMA activity on the phys channel
 */
Void ACPY2_OMAP_updateChannelStats(_DMA_Handle handle, Uns cnt)
{
    Uns chanId = 0;
    ...
    if (handle == NULL) {
        ...
        DBC_assert (chanId < _DMA4_MAX55CHANNELS);
        ...
        if ( (cnt > 0)) {
            ...
            _ChanStats[chanId].bytesPending = cnt * handle->cfn * 
                    (Int)(0x01 << (Int)(handle->csdp & DMA4_CSDP_DATATYPE_MASK));
