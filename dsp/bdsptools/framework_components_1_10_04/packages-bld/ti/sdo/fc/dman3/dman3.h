...
typedef struct DMAN3_Params {
    ...
    Uns     maxPaRamEntries;   /**< DMAN3_MAX_PARAM_ENTRIES */
     ...
     *
     * qdmaChannels: array of size DMAN3_NUM_QDMA_CHANNELS that will contain 
         * the channel numbers of the Physical QDMA channels assigned to DMAN3 via 
         * configuration. 
     */
    Uns     maxQdmaChannels;  /**< DMAN3_MAX_QDMA_CHANNELS */
    ...
    /**
     * Memory Heap descriptor for dynamic allocation of DMAN3 objects that
     ...
     *
     * A value of 0 means that IDMA3 objects will be allocated in the heap
     * specified by DMAN3_PARAMS.heapExternal. If the value of idma3Internal
     * is non-zero, IDMA3 objects will be allocated in the heap specified
     * by DMAN3_PARAMS.heapInternal.
     */
    Bool          idma3Internal;

    /**
     * Use this function for dynamic allocation of IDMA3 objects 'env' from
     * shared scratch memory. If this is NULL then the 'env' will be allocated
     * from persistent memory. If scratchFreeFxn != NULL, then this function
     * must not be NULL.
     *
     * For example, to allocate 'env' in DSKT2 scratch memory, set
     * scratchAllocFxn to DSKT2_allocScratch.
     */
    DMAN3_ScratchAllocFxn scratchAllocFxn;

    /**
     * Use this function for freeing IDMA3 objects 'env' from shared scratch
     * memory. If this is NULL then the 'env' will be freed from persistent
     * memory. If scratchAllocFxn != NULL, then this function must not be
     * NULL.
     * For example, if 'env' is to be allocated in DSKT2 scratch memory, set
     * scratchFreeFxn to DSKT2_freeScratch.
     */
    DMAN3_ScratchFreeFxn scratchFreeFxn;

    /**
     * Index of a reserved PaRam entry that will not be used for any DMA
     * transfers. This PaRam will be used to set QCHMAP register to, when
     * there is no activity on the corresponding QDMA channel.
     */
    Uns nullPaRamIndex;
